    ...
    for (record: WarcRecord <- reader.iterator()) {
      if (record.getHeader("WARC-Type").value == "response") {
        ...
        try {
          ...
          case e: Exception => logger.error("Text extraction of record '" + id + "' failed: Exception", e)
          case e: StackOverflowError => logger.error("Text extraction of record '" + id + "' failed: StackOverflowError", e)
        }
