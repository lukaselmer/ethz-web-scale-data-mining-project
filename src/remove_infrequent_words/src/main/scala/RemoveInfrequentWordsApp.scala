...
object RemoveInfrequentWordsApp {
  ...
  def main(args: Array[String]) {
    ...
    val config = new CommandLineConfigReader(conf, sys.env)
    val sc = createSparkContext(conf, config)

    val inputCombinedDirectory = config.inputCombined
    val outputDirectory = config.output
    val keepWords: Set[String] = loadKeepWords(sc, config)
  ...
  def loadKeepWords(sc: SparkContext, config: CommandLineConfigReader): Set[String] = {
    ...
    sc.textFile(config.inputWordcount)
      .map(extractCountAndWord)
      // why doesn't this work in scala :'-( minWordCount <= countAndWord._1 <= maxWordCount
      .filter(countAndWord => config.minWordCount <= countAndWord._1 && countAndWord._1 <= config.maxWordCount)
      // filter long words
